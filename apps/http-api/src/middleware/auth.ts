...
import { verifySessionToken, type SessionClaims } from "@repo/auth";
import { env } from "../env.js";
import { sendError, unauthorized } from "../http/errors.js";

/** An authenticated request carries the verified session claims. */
...
export async function verifyBearer(
  ...
  try {
    return await verifySessionToken(token, env.jwtSecret);
  } catch {
    return null;
  }
}
...
/**
 * Express middleware: require a valid session token. On success, attaches
